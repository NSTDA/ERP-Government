...
class ResPartner(models.Model):
    ...
    credit_limit_compute = fields.Monetary(
        string="Credit Limit ",
        default=-1,
        compute="_compute_credit_limit_compute",
        inverse="_inverse_credit_limit_compute",
    ...
    def _compute_credit_limit_compute(self):
        for partner in self:
            partner.credit_limit_compute = (
                self.env.company.account_default_credit_limit
                if partner.amount_credit_limit == -1
                else partner.amount_credit_limit
            )
    ...
    def _inverse_credit_limit_compute(self):
        for partner in self:
            ...
            partner.amount_credit_limit = (
                -1 if is_default else partner.credit_limit_compute
            )
